...
- **A cold menubar-json query no longer fingerprints the whole corpus before first paint when no status snapshot exists.** A deferred first-paint still cannot persist a snapshot. A complete payload is fingerprinted after the parse, so later polls keep the fast path.
